...
fn write_favorites(path: &Path, favorites: &Favorites) -> Result<(), String> {
    ...
    fs::write(&tmp_path, &content).map_err(|e| format!("Failed to write favorites: {}", e))?;
...
#[tauri::command]
pub fn is_favorite(app: AppHandle, page_title: String, command: String) -> Result<bool, String> {
