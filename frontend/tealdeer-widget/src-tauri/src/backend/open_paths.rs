...
/// 打开日志目录
#[tauri::command]
pub fn open_log_directory<R: Runtime>(app: AppHandle<R>, log_dir: String) -> Result<(), String> {
    info!("Opening log directory: {}", log_dir);

    let base_dir = settings::app_data_dir(&app)?.join("logs");
    fs::create_dir_all(&base_dir).map_err(|e| format!("Failed to create log dir: {e}"))?;

    let base_canon =
        fs::canonicalize(&base_dir).map_err(|e| format!("Failed to resolve log dir: {e}"))?;
    let target = PathBuf::from(&log_dir);
    if !target.is_absolute() {
        return Err("Log path must be absolute.".to_string());
    }
    let target_canon =
        fs::canonicalize(&target).map_err(|e| format!("Failed to resolve log path: {e}"))?;
    if !target_canon.starts_with(&base_canon) {
        return Err("Access denied: log path is outside log directory.".to_string());
    }
...
pub fn open_custom_page_file<R: Runtime>(
    ...
    if let Some(dir) = paths.custom_pages_dir {
        ...
        let file_canon =
            fs::canonicalize(&file).map_err(|e| format!("Failed to resolve file path: {e}"))?;
