...
**重要**：所有命令都必须在 **Workspace 根目录**（`tealdeer/`）执行。
...
> [!CAUTION]
> `cargo install --path tldr`只会安装 CLI 命令工具，并不会安装 GUI 软件。
...
# CI/CD 验证：构建整个 Workspace（release）
...
| 操作 | 执行路径 | 示例 |
|------|---------|------|
| 构建 CLI（默认） | Workspace 根目录 | `cd tealdeer && cargo build --release -p tldr` |
| 构建 Workspace（CI） | Workspace 根目录 | `cd tealdeer && cargo build --workspace --release` |
